...
def _build_gpt_system_prompt(language: str = "en", engine: str = "gpt") -> str:
    from string import Template

    from startupscan_api.modeling import _GPT_OUTPUT_LANGUAGE_INSTRUCTIONS
    from startupscan_api.services.ai_engines.prompt_defaults import PURPOSE_GENERATION_SYSTEM
    ...
    return Template(get_prompt_content(engine, PURPOSE_GENERATION_SYSTEM)).safe_substitute(
        output_language=output_language,
    )


def _build_gpt_user_prompt(idea_data: dict, uniqueness_key: str, engine: str = "gpt") -> str:
    from string import Template

    from startupscan_api.services.ai_engines.prompt_defaults import PURPOSE_GENERATION_USER
    from startupscan_api.services.ai_engines.prompt_store import get_prompt_content

    s = idea_data
    return Template(get_prompt_content(engine, PURPOSE_GENERATION_USER)).safe_substitute(
        ...
        market_size=s.get("market_size", ""),
        business_model=s.get("business_model", ""),
        competitive_advantage=s.get("competitive_advantage", ""),
        traction=s.get("traction", ""),
        team=s.get("team", ""),
        funding_goal=s.get("funding_goal", ""),
        use_of_funds=s.get("use_of_funds", ""),
    ...
    if model_source in ("gpt", "deepseek"):
        ...
        if api_key:
            try:
                ...
                default_model = "gpt-4.1-mini" if model_source == "gpt" else "deepseek-flash"
                ...
                response = client.chat.completions.create(
                    ...
                    messages=[
                        {"role": "system", "content": _build_gpt_system_prompt(language, engine=model_source)},
                        {"role": "user", "content": _build_gpt_user_prompt(idea_data, uniqueness_key, engine=model_source)},
                    ],
                )
        ...
        try:
            ...
            response = requests.post(
                ...
                json={
                    ...
                    "messages": [
                        {"role": "system", "content": _build_gpt_system_prompt(language, engine="ollama")},
                        {"role": "user", "content": _build_gpt_user_prompt(idea_data, uniqueness_key, engine="ollama")},
                    ],
                },
                timeout=timeout,
            )
